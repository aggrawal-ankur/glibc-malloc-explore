...
The authors have acknowledged that this layout is "misleading". Check the annotations in malloc.c.
```
/*
...
*/
```
But in my view, the layout is well-reasoned but the reasoning is not documented properly, which makes it complicated to understand.

The following is my attempt to make that reasoning visible. I have divided the explanation in four sections.
  1. Layout description (part 1)
  2. Usage description (part 1)
  3. Layout description (part 2)
  4. Usage description (part 2)

The struct layout does multiple things at once, which makes it complicated to understand. For this reason, I have divided the explanation in this way.

## Layout Description (Part 1)
...
`mchunk_prev_size` holds the size of the previous chunk and `mchunk_size` holds the size of the current chunk.
  ...
  - **Note2: INTERNAL_SIZE_T is discussed later in section#. For the time being, treat it like `size_t`.**
  - **Note3: The existence of mchunk_prev_size is discussed later in section#.**
...
This is how malloc_chunk is used, but we are not done yet.

## Layout Description (Part 2)

A standard Linux process calls malloc and free several times. This repeated allocation-deallocation fragments the memory.
...
Suppose two chunks of 48 bytes were freed. Now we have 96 bytes of memory which can be reused. The next malloc request asked for 96 bytes. Can we reuse the 96 bytes? No. Because, those 96 bytes are not contiguous. That's external fragmentation in layout1.

Suppose two adjacent chunks, each of size 48 bytes, were freed and the next malloc request asked for 96 bytes. Can we reuse these 96 bytes? NO. Because, the memory is contiguous yet fragmented across two chunks. That's external fragmentation in layout2.

Fragmentation increases the pressure on physical memory because the freed chunks are still backed by physical memory but not utilized by the process. The only way to reduce this pressure is to reallocate the freed chunks, which entirely depends on the process asking for a size which is available as a free chunk.
...
**Note: Right now, only the 0th bit is important for us. The remaining two bits are discussed later.**
...
How about putting another size field in the front of malloc_chunk and use it as a property of the previous chunk?
  ...
  - There will be some sort of dummy chunk in the end to compensate for the last malloc-ed chunk.

The layout would look something like this:
```
Structurally -> [ Chunk1                                          ] [ Chunk2                                          ] [ Dummy Chunk                                     ]
                --------------------------------------------------- --------------------------------------------------- ---------------------------------------------------
                | prev_s | mchunk_s | fd | bk | fd_next | bk_next | | prev_s | mchunk_s | fd | bk | fd_next | bk_next | | prev_s | mchunk_s | fd | bk | fd_next | bk_next |
                --------------------------------------------------- --------------------------------------------------- ---------------------------------------------------
Functionally ->          [ Chunk1                                            ] [ Chunk2                                          ]
```

***The mchunk_prev_size of the n<sup>th</sup> chunk is "by-use" a part of the (n-1)<sup>th</sup>chunk. Structurally, it is still a part of the n<sup>th</sup> chunk.*** This is boundary tag method in my words.
...
## Usage Description (Part 2)

Everything in glibc-malloc is directly or indirectly connected with size. Therefore, the remaining description is about the **allocator's size model**.

The size model is described using macros. It contains two types of macros.
  1. Macros which resolve to certain numeric values.
  2. Macros that simply provide a block of code a name.
...
This will land us at the `fd` field in the struct, where the payload memory starts in an in-use chunk.
